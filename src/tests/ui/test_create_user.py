...
class TestCreateUser:
    ...
    def test_create_user_with_valid_data(self, api_manager: ApiManager, page: Page, user_request: CreateUserRequest):
        # Create User
        AdminPage(page).open().should_be(Condition.visible, AdminPage(page).admin_panel_text).create_user(
    ...
    def test_create_user_with_invalid_data(
        ...
        AdminPage(page).open().should_be(Condition.visible, AdminPage(page).admin_panel_text).create_user(
            username, password
        ).check_alert_message_and_accept(expected_error).should_not_have_user(api_manager, username)
