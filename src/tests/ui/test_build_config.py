...
class TestBuildConfiguration:
    ...
    def test_cancel_build_configuration_creation(
        ...
        ProjectsPage(page).open().click_new_build_configuration(
            created_project.name, created_project.id
        ).click_cancel().should_not_have_build_configuration(build_config_name)
    ...
    def test_pause_and_activate_build_configuration(
        ...
        assert api_manager.admin_steps.get_buildtype_paused_status(
            build_config.id
        ), f"Expected build config '{build_config.id}' to be paused via API"

        build_config_page.activate_build_configuration().should_not_be_paused()

        assert not api_manager.admin_steps.get_buildtype_paused_status(
            build_config.id
    ...
    def test_add_build_step(self, page: Page, api_manager: ApiManager, build_config):
        ...
        EditBuildRunnersPage(page).open(build_config.id).add_command_line_step(
